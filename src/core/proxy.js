...
/**
 * Singleton class for proxy management
 */
class ProxyManager {
  constructor() {
    // Initialize with empty values - populated on initialization
    this.proxies = [];
    this.config = {};
    this.walletNum = null;
  ...
  async loadProxies() {
    try {
      const proxyPath = path.join(process.cwd(), 'data', 'proxy.txt');
      
      try {
        const proxyFile = await fs.readFile(proxyPath, 'utf8');
        ...
        if (err.code === 'ENOENT') {
          ...
          await fs.writeFile(proxyPath, '', 'utf8');
        } else {
          throw err;
        }
      }
